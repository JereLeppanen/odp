					...
					uint32_t init_val)
{
	...
	for (i = 0; i < data_len / 8; i++) {
		init_val =
			(uint32_t)__builtin_ia32_crc32di(init_val,
							 *(const odp_una_u64_t ODP_MAY_ALIAS *)pd);
		pd += 8;
	}

	if (data_len & 0x4) {
		init_val =
			__builtin_ia32_crc32si(init_val, *(const odp_una_u32_t ODP_MAY_ALIAS *)pd);
		pd += 4;
	}
#else
	...
	if (data_len & 0x2) {
		init_val =
			__builtin_ia32_crc32hi(init_val, *(const odp_una_u16_t ODP_MAY_ALIAS *)pd);
		pd += 2;
	}
