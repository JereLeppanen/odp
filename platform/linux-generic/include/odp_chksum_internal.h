...
static uint64_t chksum_partial(const void *addr, uint32_t len, uint32_t offset)
{
	 ...
	 *
	 * Due to the type-punning in this function, ODP_MAY_ALIAS is
	 * needed to prevent the compiler from making assumptions about
	 * aliasing.
	 */
	typedef odp_una_u32_t ODP_MAY_ALIAS x_uint32_t;
	typedef odp_una_u16_t ODP_MAY_ALIAS x_uint16_t;
#else
	/*
	 * In this case alignment is not a problem, since we align
	 * manually. But ODP_MAY_ALIAS is still needed due to the
	 * type-punning.
	 */
	typedef uint32_t ODP_MAY_ALIAS x_uint32_t;
	typedef uint16_t ODP_MAY_ALIAS x_uint16_t;
#endif
	const x_uint16_t *w;
	const x_uint32_t *d;
	uint64_t sum = 0;
